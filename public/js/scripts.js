...
//Inicializo el Editor de Noticias
tinyMCE.init(
    {
     mode : "textareas",
     language: 'es',
     ...
     content_css: ['material-classic','//www.tiny.cloud/css/codepen.min.css'],
     icons: 'material',
    ...
    for (let index=0; index < jsonMenu.length; index++) {
     ...
     let ulCollapsible=document.createElement("ul");
        ...
        for (let index2=0; index2 < jsonMenu[index].publicaciones.length; index2++) { 
         ...
         //Defino el Link correctamente publicacion.id
         let cadenaEnlaces="/publicacion/ver/"+jsonMenu[index].publicaciones[index2].id;
...
//Funcion para obtener el Menu via ApiRest
function obtenerMenuPrincipal(urlApi) {
     ...
     type: 'POST', 
       url: urlApi,           
        ...
        success: function(response) {
         //mostrarAlerta('Se obtuvo el menu de forma exitosa');
         //console.log(response);         
         renderizarMenuPrincipal(response); 
         renderizarMenuMovil(response)          
        },
